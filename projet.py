...
class Person:
    ...
    # create a text representation of a person information (used automatically when calling ui.curses_print on a person object)
    def __str__(self):
        string =100*'-'+'\n'
    ...
    # ui.curses_print current and all its descendant attached childs (distincts)    
    def display_tree(self):
        found = self.get_all_persons_distincts()
        if found:
            for person in found:
                ui.curses_print(person)

    # ui.curses_print attached childs to current person
    def display_children(self):
        ui.curses_print('children of',self.firstname,self.lastname)
        for child in self.children:
            ui.curses_print(child)

    # ui.curses_print attached father and mother to current person
    def display_parents(self):
        ui.curses_print('Parents of',self.firstname,self.lastname)
        if self.father!=None:
            ui.curses_print('Father:',self.father)
        else: ui.curses_print('Father: None')
        if self.mother!=None:
            ui.curses_print('Mother:',self.mother)
        else: ui.curses_print('Mother: None')  
...
class Trees:
    ...
    # ui.curses_print distincts person in the whole Genealogique data structure
    def display(self):
        for person in self.get_all_persons_distincts():
            ui.curses_print(str(person))
    ...
    def delete_person(self,person): 
        ui.curses_print('deleting:',person.firstname,person.lastname)
...
def create_person(trees):
    fname=ui.curses_input('votre prenom:')
    lname=ui.curses_input('votre nom:')
    address=ui.curses_input('adresse:')
    nationality=ui.curses_input('nationalité:')
    phone=ui.curses_input('telephone')
    birthday=ui.curses_input('date de naissance:')
    fathername=ui.curses_input('prenom du pere:')
    fatherbirthday=ui.curses_input('date de naissance du pere:')
    motherfirstname=ui.curses_input('prenom de la mere:')
    motherlastname=ui.curses_input('nom de la mere:')
    motherbirthday=ui.curses_input('date de naissance de la mere:') 
...
def identify_persone(trees):
    isnew = ui.curses_input("vous etes deja inscrit dans l'arbre (o/n):")
    ...
    if isnew=='o':
        ...
        if persone:
            ui.curses_print('Inscription avec succes')
        return persone
...
def add_direct_parent(trees,person):
    ui.curses_print('add_direct_parent')
    proposition_peres=trees.search_person(person.fathername,person.lastname,None)
    ui.curses_print('proposition_peres')
    proposition_meres=trees.search_person(person.motherfirstname,person.motherlastname,None)
    ui.curses_print('proposition_meres')
    choix1=choix2=0
    if proposition_peres:
        ui.curses_print('Proposition de pere')
        i=1
        for pere in proposition_peres:
            ui.curses_print(i,'-',pere.firstname,pere.lastname,pere.birthday)
            i+=1
        ui.curses_print("Choisir votre pere (0 pour rien):")
        choix1 = int(ui.curses_input('votre choix:'))
        if choix1!=0:
            person.attach_father(proposition_peres[choix1-1])

    if proposition_meres:
        ui.curses_print('Proposition de mere')
        i=1
        for mere in proposition_meres:
            ui.curses_print(i,'-',mere.firstname,mere.lastname,mere.birthday)
            i+=1
        ui.curses_print("Choisir votre mere (0 pour rien):")
        choix2 = int(ui.curses_input('votre choix:'))
        if choix2!=0:
...
def  add_person_child(trees,person):
    ...
    if child_proposition:
        ui.curses_print("Proposition d'enfants")
        i=1
        for child in child_proposition:
            ui.curses_print(i,'-',child.firstname,child.lastname,child.birthday)
            i+=1
        ui.curses_print("Choisir votre enfant (0 pour rien):")
        choix = int(ui.curses_input('votre choix:'))
        if choix!=0:
...
def delete_direct_parent(trees,current_user):
    ...
    if current_user.father!=None:
        ui.curses_print("Votre pere est:")
        ui.curses_print(current_user.father)
        choix=ui.curses_input('Voulez vous le supprimer (o/n)?')
        if choix=='o':
            current_user.father=None
            ui.curses_print("\n>>>>>>>>>>>>> Supression du pere avec success \n\n")
    if current_user.mother!=None:
        ui.curses_print("Votre mere est:")
        ui.curses_print(current_user.mother)
        choix=ui.curses_input('Voulez vous la supprimer (o/n)?')
        if choix=='o':
...
def delete_children(trees,current_user):
    if current_user.attached_children()==None or len(current_user.attached_children())==0:
        ...
        return
    ui.curses_print('Vos enfants sont:')
    i=1
    for child in current_user.attached_children():
        ui.curses_print(i,'-',child)
        i+=1
    ui.curses_print("Choisir l'enfant a supprimer (0 pour rien):")
    choix = int(ui.curses_input('votre choix:'))
...
def filter_by_attachment(trees):
    ui.curses_print('\t\t\t1- Afficher les personnes avec attachement de mere')
    ui.curses_print('\t\t\t2- Afficher les personnes avec attachement de pere')
    ui.curses_print("\t\t\t3- Afficher les personnes avec attachement d'enfants")
    ui.curses_print('\n\t\t(0- pour revenir)')
    choice = int(ui.curses_input('Donner le numero de votre choix:'))
    ...
    ui.curses_print('Personnes trouvées:')
    if len(found)>0:
        for person in found:
            ui.curses_print(person)
    else: ui.curses_print('Aucun')
...
def choose_main_menu(personne):
    ui.curses_print("\tBienvenu a l'application << Arbre généalogique >>")
    ui.curses_print("\tvotre identite:",personne.firstname,personne.lastname,"Date de naissance(",personne.birthday,")\n\n")

    ui.curses_print('\t\t Ajout ')
    ui.curses_print('\t\t\t1 - Un parent direct') 
    ui.curses_print('\t\t\t2 - Un enfant') 
    ui.curses_print('\t\t Suppression')
    ui.curses_print('\t\t\t3 - Elle-même') 
    ui.curses_print('\t\t\t4 - Un parent direct') 
    ui.curses_print('\t\t\t5 - Un enfant') 
    ui.curses_print('\t\t Consultation')
    ui.curses_print("\t\t\t6 - L’ arbre généalogique global") 
    ui.curses_print("\t\t\t7 - La partie de l’arbre généalogique de sa famille") 
    ui.curses_print("\t\t\t8 - Sa descendance") 
    ui.curses_print("\t\t\t9 - Son ascendance") 
    ui.curses_print("\t\t\t10- Sa descendance et son ascendance")
    ui.curses_print("\t\t\t11- Les personnes ayant un lien de parenté donné") 
    ui.curses_print('\t\t\t12- La liste des personnes sans ascendant')
    ui.curses_print('\t\t\t13- La liste des personnes sans descendant') 
    ui.curses_print('\t\t\t14- La liste des personnes ayant le plus grand nombre d’ascendants vivants')
    ui.curses_print('\n\t\t(0- pour quitter)')

    choice = int(ui.curses_input('Donner le numero de votre choix:'))
    while choice <0 or choice > 14:
        ui.curses_print('Erreur de saisie, donner un choix entre 1 et 14')
        choice = int(ui.curses_input('Donner le numero de votre choix:'))
...
if not current_user:
    ui.curses_print("Vous n'etes pas inscrit dans l'arbre, veulleiz quitter ...")
    exit(0)
...
while choice!=0:
    if choice==1:
        ...
        ui.curses_print("Personce avec la plus longue liste d'ascendant:")
        ui.curses_print(trees.persons_longest_parent_path())
    ui.wait()
    choice = ui.display_menu()
    # choice = choose_main_menu(current_user)
